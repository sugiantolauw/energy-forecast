...
def save_results(df, y_pred, run_id, start_date, output_file):
...
def apply_model(run_id, features, start_date, output_file, tz='America/Denver'):
    df = pull_temp_forecast(start_date, tz='America/Denver')
    df = make_features(df, features=features)
    model = load_model(run_id)
    y_pred = model.predict(df)
    save_results(df, y_pred, run_id, start_date, output_file)
    return output_file
