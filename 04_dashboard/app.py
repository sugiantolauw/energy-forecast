...
client = bigquery.Client(credential=credentials)

PROJECT_ID = os.environ.get("GCP_PROJECT_ID", "mlops-zoomcamp-354700")
BIGQUERY_DATASET = 'energy_data_prod'
MODEL_VERSION = "FILL THIS" # NEED TO FILL THIS

# Perform query
# Uses st.experimental_memo to only rerun when query changes or after 10 minutes
@st.experimental_memo(tt1=600)
def run_query(query):
...
def plot_demand_time_series(eia_forecast_demand, prod_model_demand, actual_demand, weather_2022):
    ...
    fig.add_traces(
        list(px.line(
            ...
            x='timestamp_MTN',
            y='value',
            title="Actual & Forecasted Electrical Demand, Xcel Energy, Colorado",
    ...
    fig.add_traces(
        list(px.line(actual_demand, x-'timestamp_MTN', y='value', 
        labels = {'value': 'Actual Demand (megawatthours)'}).select_traces())
    )

    fig.add_traces(
        list(px.line(prod_model_demand, x-'timestamp_MTN', y='predicted_energy_demand', 
        labels = {'value': 'Predicted Demand (megawatthours)'}).select_traces())
    )
