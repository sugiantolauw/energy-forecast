...
# EIA series ID's taht will be downloaded by the DAG
SERIES_LIST = [
    'EBA.PSCO-ALL.D.H', # Electrical demand public service company of colorado in UTC
    'EBA.PSCO-ALL.DF.H' # Day-ahead demand forecast for public service company of colorado, hourly in UTC
]

def download_energy_demand_json(series_id, outfile):
    """
    Request data for an EIA series and save result locally as json file
    """
...
def extract_energy_demand_data(series_id, local_file_name, local_file_suffix):
    ...
    # The schema must be defined for data_df to store table as parquet.
    # Pandas/Pyarrow cant automatically define the schema. The value field is mostly 
    # integers, but does have missing values. Automatically defining the schema
    # will assign int which cant contain missing values
    fields = [
...
with DAG(
    ...
    with TaskGroup(group_id='download_and_extract') as dl_and_extract_tg:
        for series_id in SERIES_LIST:
            ...
            # Delete all files downloaded to worker
    ...
    with TaskGroup(group_id = 'load_to_bq') as load_to_bq_tg:
        ...
        for bucket_subfolder, external_table, native_table in zip(bucket_subfolders,
            ...
            gcs_to_bq_ext_task =BigQueryCreateExternalTableOperator(
                task_id = "gcs_to_bq_ext_eia_series_{bucket_subfolder}_task",
                table_resource = {
                    "tableReference": {
